...
const Aisles = ({ aisle, setRackId }) => {
  ...
  // Lọc các racks có aisle tương ứng
  const filteredRacks = racks.filter(rack => rack.aisle === aisle.id);
  
  // Tính số lượng racks có trong aisle
  const count = filteredRacks.length;

  return (
    <Box display={'flex'} alignItems={'center'} height={'100%'} background={'gray'} borderRadius={'4px'} position={'relative'}>
      <Box 
        width={5} 
        background={'#e7ecf2'} 
        position={'absolute'} 
      ...
      <Box marginLeft={'33px'} display={'flex'} gap={"20px"} flexWrap={'wrap'} padding={"15px"}>
